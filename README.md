...
- **支持剪映最新版本** - 与剪映专业版保持同步，支持最新特性 (验证版本：剪映 v10.1.0)
- **离线剪映草稿** - 草稿包含所有的资源，无需等待官方下载
- **文本模板** - 丰富的官方文字动画模板，一键应用动态效果 (支持 8000+文本模板)
...
``` bash
# 请使用以下命令添加 skills
npx skills add duoec/duo-video
```

| Skills                | 描述                                                         |
| --------------------- | ------------------------------------------------------------ |
| video-project-creator | 视频工程创建器，生成VideoProject视频工程json                 |
| video-exporter        | 将 VideoProject 工程导出成 剪映草稿，生成成品视频            |
| video-api             | 通过HTTP API的方式，生成VideoProject视频工程json，导出剪映工程，生成成品视频 |
