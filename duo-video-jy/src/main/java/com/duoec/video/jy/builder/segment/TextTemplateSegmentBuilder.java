...
public class TextTemplateSegmentBuilder extends BaseSegmentBuilder<TextTemplateMaterial> {
    ...
    public Segment build(JianyingProjectBuildState state, VideoScript videoScript, VideoSegment videoSegment, TextTemplateMaterial material) {
        ...
        Track textTrack = JianyingTrackBuilder.getOrCreateTrack(state.getJianyingProject(), Track.TYPE_TEXT, "文本模板", segmentTime.getStart(), segmentTime.calculateEndTime());

        Segment segment = getSegment(state.getJianyingProject(), videoSegment, textTemplate)
