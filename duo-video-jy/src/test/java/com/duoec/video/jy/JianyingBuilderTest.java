...
import com.duoec.video.builder.VideoProjectBuilder;
...
class JianyingBuilderTest extends BaseTest {
    ...
    void buildWithProjectBuilder() {
        ...
        VideoProject videoProject = VideoProjectBuilder.createBuilder(SnowflakeIdUtils.nextTmpId(), "测试", 1080, 1920)
                .setTest(true) // 设置为测试模式
                .getTextTemplateBuilder(0) // 进入第一个分镜
                .add(textTemplateResourceId, "太好了", 0, 3000) // 添加一个文本模板
                .setPosition(0, -400) // 指定展示位置，0，0表示视频中央 上正下负 左负右正
                .back() // 返回到 VideoProjectBuilder

                .getTextBuilder(0)
                .add(String.valueOf(textTemplateResourceId), 0, 3000)
                .setStyle(
                        new TextStyle()
                                .setFontSize(5)
                                .setTextAlign(1)
